...
<head>
  ...
  <style>
    ...
    .shift-item .time { font-weight: var(--font-semibold); min-width: 100px; }
    .shift-item .name { color: var(--color-text-secondary); flex: 1; }
    ...
    function showDayDetail(dateStr) {
      ...
      document.getElementById('shiftList').innerHTML = shifts.length === 0
        ...
        : shifts.map(shift => {
            ...
            return `<div class="shift-item" style="border-left-color:${shift.color || '#3788d8'}"><span class="time">${start} - ${end}</span><span class="name">${shift.title || '未設定'}</span><span class="color-indicator" style="background:${shift.color || '#3788d8'}"></span></div>`;
          }).join('');
    }
